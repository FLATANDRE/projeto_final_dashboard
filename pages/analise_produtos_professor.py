...
# estao armazenados nesta variavel
dados_vendas = pd.read_csv('dados/vendas.csv')

st.title("📦 Análise de Produtos - Professor")

# TODO: melhorar as opções do selectbox, para mostrar os produtos disponiveis no dataframe

# armazena na memoria do computador
# a opção selecionada pelo usuario
option = st.selectbox(
    "Selecione um produto",
    ("Headphone", "Headset", "Memória RAM", "Mouse","SSD","Teclado","Webcam"),
)

# quero filtrar os dados de vendas usanedo a opção selecionada pelo usuário
...
with colA:
    ...
    # processo de DEBUG do codigo
    #st.dataframe(df_agrupado)

    fig = px.bar(df_agrupado,x='Região',y='Vendas',title=f'Vendas por Região - {option}',color='Vendas')
    st.plotly_chart(fig, width='stretch')
    

with colB:
    df_agrupado2 = dados_filtrados.groupby('Vendedor')['Vendas'].sum().reset_index()
    fig = px.pie(df_agrupado2,values='Vendas',names='Vendedor',title='Vendas por Vendedor')
    st.plotly_chart(fig, width='stretch')
...
fig = px.area(df_agrupado3,x='Mês',y='Vendas',title=f'Evolução Mensal de {option}')
st.plotly_chart(fig, width='stretch')