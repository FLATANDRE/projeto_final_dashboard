...
# Filtro de período
# O filtro de data é configurado para permitir a seleção de um intervalo 
# entre a data mínima e máxima presente nos dados
...
# Vendas por vendedor
# A análise de performance por vendedor é realizada agrupando os dados filtrados pelo nome 
# do vendedor 
# e calculando as seguintes métricas:
...
# Os resultados são arredondados para 2 casas decimais e ordenados pela receita em ordem 
...
# cria um gráfico de barras para visualizar a receita por vendedor, ordenando os vendedores
# pela receita em ordem decrescente para destacar os vendedores com melhor performance
...
# A função dt.to_period('M') converte a data para um período mensal, e astype(str) converte esse 
# período # para uma string no formato 'YYYY-MM'.
# Em seguida, os dados são agrupados por essa nova coluna 'Mês' para calcular a receita e o 
# lucro total para cada mês, resultando em um dataframe mensal que pode ser usado para 
# análise temporal.
...
# Cria um gráfico de barras para comparar a receita e o lucro mensal, usando a coluna 'Mês'
# no eixo x e as colunas 'Receita' e 'Lucro' no eixo y. O parâmetro barmode='group' é usado 
# para exibir as barras de receita e lucro lado a lado para cada mês, facilitando a comparação 
# visual entre as duas métricas ao longo do tempo. O título do gráfico é definido 
# como 'Receita x Lucro Mensal'.
fig = px.bar(
...
# xaxis_tickangle=-45 é utilizado para rotacionar os rótulos do eixo
# x em um ângulo de -45 graus, o que pode ajudar a melhorar a legibilidade dos rótulos,
# especialmente quando há muitos meses ou quando os rótulos são longos. Essa rotação
# evita que os rótulos se sobreponham e torna o gráfico mais fácil de interpretar.
fig.update_layout(xaxis_tickangle=-45)

# Exibe o gráfico usando Streamlit, com a largura configurada 
# para se estender ao máximo do contêiner disponível.
st.plotly_chart(fig, width='stretch')


# Tabela de dados
# Realiza o download dos dados filtrados em formato CSV, permitindo que os 
# usuários baixem os dados
